...
task.spawn(function()
    ...
    if not (isfolder and makefolder and writefile and readfile) then return end
...
local function getSongFiles()
	...
	-- フォルダがなければ作成する
	if isfolder and makefolder and not isfolder(targetFolder) then 
		pcall(function() makefolder(targetFolder) end) 
	end

	if not listfiles then
		return {"listfiles非対応環境"}
	end
	...
	-- パス形式の試行 (一部のエクゼキューター向け)
	if not success or type(allFiles) ~= "table" then
		success, allFiles = pcall(function() return listfiles("./" .. targetFolder) end)
	end
	...
	for _, filePath in ipairs(allFiles) do
		if type(filePath) == "string" and filePath:lower():match("%.json$") then
...
local songDropdown = PianoSongSec:AddDropdown({
	...
	Callback = function(v)
		...
		local success, fileContent

		if readfile then
			success, fileContent = pcall(function() return readfile(filePath) end)
			if not success then
				-- 別のパス形式で再試行
				success, fileContent = pcall(function() return readfile("./" .. filePath) end)
			end
