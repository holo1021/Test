...
local function applyPinkGradientEffect()
    ...
    local function getPinkGradientColor(t)
        -- 薄めピンクからこゆめピンクへの3色グラデーション
        local pinkColors = {
            Color3.fromRGB(255, 182, 193), -- ライトピンク（薄め）
            Color3.fromRGB(255, 105, 180), -- ホットピンク（中間）
            Color3.fromRGB(219, 112, 147)  -- パールピンク（こゆめ）
        }
        
        -- スムーズなグラデーションのための補間
        local position = (math.sin(t * 2) + 1) / 2  -- 0から1の間を往復
    ...
    RunService.RenderStepped:Connect(function(dt)
        time = time + dt * 0.5  -- 速度調整（0.5倍速）
        
        for i = #strokes, 1, -1 do
            local stroke = strokes[i]
            if stroke and stroke.Parent then
                -- 各UIStrokeごとに少しずつ位相をずらす
...
-- Orion UIのカラー設定
local Theme = {
    BackgroundColor = Color3.fromRGB(139, 69, 19),  -- ブラウン
    SliderColor = Color3.fromRGB(255, 105, 180),     -- ホットピンク
    TextColor = Color3.fromRGB(255, 255, 255),       -- ホワイト
    SectionColor = Color3.fromRGB(210, 180, 140),    -- ライトブラウン
}
...
local FerrisWheelConfig = {
    ...
    -- 固定方向設定
    FixedDirection = true,  -- 固定方向を使用するかどうか
    FixedYaw = 0,           -- 固定ヨー角（度）- 水平方向の向き
    FixedPitch = 0,         -- 固定ピッチ角（度）- 上下方向の向き
    FixedRoll = 0,          -- 固定ロール角（度）- 回転方向の向き
}
...
local function changeObjectID(id)
    ...
    OrionLib:MakeNotification({
        Name = "オブジェクトID変更", Content = id .. " に切り替えました", Time = 3
    })
end
...
local function toggleFeather(state)
    ...
    if state then
        ...
        startFeatherLoop()
        OrionLib:MakeNotification({ Name = "フェザー開始", Content = "オブジェクト数: " .. #FeatherAssignedToys, Time = 3 })
    else
        stopFeatherLoop()
...
local function toggleMagicCircle(state)
    ...
    if state then
        ...
        OrionLib:MakeNotification({ Name = "魔法陣開始", Content = "高さ: " .. MagicCircleConfig.Height .. ", 直径: " .. MagicCircleConfig.Diameter, Time = 3 })
    else
        stopMagicCircleLoop()
...
local function toggleHeart(state)
    ...
    if state then
        ...
        startHeartLoop()
        OrionLib:MakeNotification({ Name = "ハート開始", Content = "サイズ: " .. HeartConfig.Size .. ", 数: " .. HeartConfig.ObjectCount, Time = 3 })
    else
        stopHeartLoop()
...
local function toggleBigHeart(state)
    ...
    if state then
        ...
        startBigHeartLoop()
        OrionLib:MakeNotification({ Name = "ビッグハート開始", Content = "サイズ: " .. BigHeartConfig.Size .. "×" .. BigHeartConfig.HeartScale, Time = 3 })
    else
        stopBigHeartLoop()
...
local function toggleStar(state)
    ...
    if state then
        ...
        startStarLoop()
        OrionLib:MakeNotification({ Name = "スター開始", Content = "外側半径: " .. StarConfig.OuterRadius, Time = 3 })
    else
        stopStarLoop()
...
local function toggleStar2(state)
    ...
    if state then
        ...
        startStar2Loop()
        OrionLib:MakeNotification({ Name = "スター2開始", Content = "サイズ: " .. Star2Config.Size, Time = 3 })
    else
        stopStar2Loop()
...
local function toggleSphere(state)
    ...
    if state then
        ...
        startSphereLoop()
        OrionLib:MakeNotification({ Name = "球体開始", Content = "半径: " .. SphereConfig.Radius, Time = 3 })
    else
        stopSphereLoop()
...
local function toggleFerrisWheel(state)
    ...
    if state then
        ...
        OrionLib:MakeNotification({ Name = "観覧車開始", Content = "半径: " .. FerrisWheelConfig.Radius .. ", オブジェクト数: " .. #FerrisWheelAssignedToys, Time = 3 })
    else
        stopFerrisWheelLoop()
    ...
    AnimN1LoopConn = RunService.RenderStepped:Connect(function(dt)
        ...
        for i, point in ipairs(AnimN1Points) do
            if point.assignedToy and point.assignedToy.BP and point.assignedToy.BG then
                ...
                if point.part then point.part.Position = targetPosition end
                toy.BP.Position = targetPosition
                toy.BG.CFrame = CFrame.new(targetPosition) -- 向き固定 (カオス感を出すため)
            end
        end
    end)
end
...
local function toggleAnimN1(state)
    ...
    if state then
        ...
        startAnimN1Loop()
        OrionLib:MakeNotification({ Name = "N1: カオス・サークル開始", Content = "オブジェクト数: " .. #AnimN1AssignedToys, Time = 3 })
    else
        stopAnimN1Loop()
        OrionLib:MakeNotification({ Name = "N1: カオス・サークル停止", Content = "アニメーション解除", Time = 2 })
    end
end
    ...
    AnimN2LoopConn = RunService.RenderStepped:Connect(function(dt)
        ...
        for i, point in ipairs(AnimN2Points) do
            if point.assignedToy and point.assignedToy.BP and point.assignedToy.BG then
                ...
                if point.part then point.part.Position = targetPosition end
                toy.BP.Position = targetPosition
                toy.BG.CFrame = CFrame.new(targetPosition) -- 向き固定
            end
        end
    end)
end
...
local function toggleAnimN2(state)
    ...
    if state then
        ...
        startAnimN2Loop()
        OrionLib:MakeNotification({ Name = "N2: トルネード・スパイラル開始", Content = "オブジェクト数: " .. #AnimN2AssignedToys, Time = 3 })
    else
        stopAnimN2Loop()
        OrionLib:MakeNotification({ Name = "N2: トルネード・スパイラル停止", Content = "アニメーション解除", Time = 2 })
    end
end
...
local function startAnimN3Loop()
    ...
    AnimN3LoopConn = RunService.RenderStepped:Connect(function(dt)
        ...
        local explosionFactor = (cycle + 1) / 2 -- 0から1の間で変動
...
local function toggleAnimN3(state)
    ...
    if state then
        ...
        startAnimN3Loop()
        OrionLib:MakeNotification({ Name = "N3: ハイパー・エクスプロージョン開始", Content = "オブジェクト数: " .. #AnimN3AssignedToys, Time = 3 })
    else
        stopAnimN3Loop()
        OrionLib:MakeNotification({ Name = "N3: ハイパー・エクスプロージョン停止", Content = "アニメーション解除", Time = 2 })
    end
end
...
local function executeScript(url, scriptName)
    ...
    if success then
        ...
        OrionLib:MakeNotification({
            Name = scriptName,
            Content = "読み込み失敗: " .. tostring(result),
            Time = 5
        })
    end
end
...
ObjectIDTab:AddLabel("対応オブジェクト数: " .. #ObjectIDConfig.AvailableObjects .. "種類")
...
FeatherTab:AddSlider({ Name = "最大オブジェクト数", Min = 2, Max = 100, Default = FeatherConfig.maxSparklers, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) FeatherConfig.maxSparklers = v end })
FeatherTab:AddSlider({ Name = "オブジェクト間隔", Min = 1, Max = 20, Default = FeatherConfig.spacing, Color = Theme.SliderColor, Increment = 0.5, ValueName = "スタッド", Callback = function(v) FeatherConfig.spacing = v end })
FeatherTab:AddSlider({ Name = "高さオフセット", Min = -10, Max = 30, Default = FeatherConfig.heightOffset, Color = Theme.SliderColor, Increment = 0.5, ValueName = "スタッド", Callback = function(v) FeatherConfig.heightOffset = v end })
FeatherTab:AddSlider({ Name = "背面オフセット", Min = 0, Max = 30, Default = FeatherConfig.backwardOffset, Color = Theme.SliderColor, Increment = 0.5, ValueName = "スタッド", Callback = function(v) FeatherConfig.backwardOffset = v end })
FeatherTab:AddSection({ Name = "角度設定" })
FeatherTab:AddSlider({ Name = "オブジェクト傾き角度", Min = 0, Max = 90, Default = FeatherConfig.tiltAngle, Color = Theme.SliderColor, Increment = 5, ValueName = "度", Callback = function(v) FeatherConfig.tiltAngle = v end })
FeatherTab:AddSection({ Name = "上下動設定" })
FeatherTab:AddSlider({ Name = "上下動速度", Min = 0, Max = 20, Default = FeatherConfig.waveSpeed, Color = Theme.SliderColor, Increment = 0.5, ValueName = "速度", Callback = function(v) FeatherConfig.waveSpeed = v end })
FeatherTab:AddSlider({ Name = "基本振幅", Min = 0, Max = 20, Default = FeatherConfig.baseAmplitude, Color = Theme.SliderColor, Increment = 0.5, ValueName = "スタッド", Callback = function(v) FeatherConfig.baseAmplitude = v end })
...
MagicCircleTab:AddToggle({ Name = "発光効果", Default = MagicCircleConfig.GlowEffect, Callback = function(v) MagicCircleConfig.GlowEffect = v; if MagicCircleConfig.Enabled then toggleMagicCircle(false); task.wait(0.1); toggleMagicCircle(true) end end })
MagicCircleTab:AddSlider({ Name = "高さ", Min = 0, Max = 50, Default = MagicCircleConfig.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) MagicCircleConfig.Height = v end })
MagicCircleTab:AddSlider({ Name = "直径", Min = 5, Max = 50, Default = MagicCircleConfig.Diameter, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) MagicCircleConfig.Diameter = v end })
MagicCircleTab:AddSlider({ Name = "オブジェクト数", Min = 3, Max = 100, Default = MagicCircleConfig.ObjectCount, Color = Theme.SliderColor, Increment = 1, ValueName = "個", Callback = function(v) MagicCircleConfig.ObjectCount = v; if MagicCircleConfig.Enabled then rescanMagicCircle() end end })
MagicCircleTab:AddSection({ Name = "回転設定" })
MagicCircleTab:AddSlider({ Name = "回転速度", Min = 1, Max = 100, Default = MagicCircleConfig.RotationSpeed, Color = Theme.SliderColor, Increment = 1, ValueName = "速度", Callback = function(v) MagicCircleConfig.RotationSpeed = v end })
MagicCircleTab:AddButton({ Name = "オブジェクト再スキャン", Callback = function() if MagicCircleConfig.Enabled then rescanMagicCircle() end end })
...
HeartTab:AddToggle({ Name = "ハート起動", Default = false, Callback = toggleHeart })
HeartTab:AddToggle({ Name = "プレイヤー追従", Default = HeartConfig.FollowPlayer, Callback = function(v) HeartConfig.FollowPlayer = v end })
HeartTab:AddSection({ Name = "サイズ設定" })
HeartTab:AddSlider({ Name = "ハートサイズ", Min = 2, Max = 50, Default = HeartConfig.Size, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) HeartConfig.Size = v end })
HeartTab:AddSlider({ Name = "基本高さ", Min = 0, Max = 50, Default = HeartConfig.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) HeartConfig.Height = v end })
HeartTab:AddSlider({ Name = "オブジェクト数", Min = 6, Max = 100, Default = HeartConfig.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) HeartConfig.ObjectCount = v; if HeartConfig.Enabled then toggleHeart(false); task.wait(0.1); toggleHeart(true) end end })
HeartTab:AddSection({ Name = "動き設定" })
HeartTab:AddSlider({ Name = "回転速度", Min = 0, Max = 10, Default = HeartConfig.RotationSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) HeartConfig.RotationSpeed = v end })
HeartTab:AddSlider({ Name = "脈動速度", Min = 0, Max = 10, Default = HeartConfig.PulseSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) HeartConfig.PulseSpeed = v end })
HeartTab:AddSlider({ Name = "脈動振幅", Min = 0, Max = 10, Default = HeartConfig.PulseAmplitude, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) HeartConfig.PulseAmplitude = v end })
...
BigHeartTab:AddToggle({ Name = "おっきぃハート起動", Default = false, Callback = toggleBigHeart })
BigHeartTab:AddToggle({ Name = "プレイヤー追従", Default = BigHeartConfig.FollowPlayer, Callback = function(v) BigHeartConfig.FollowPlayer = v end })
BigHeartTab:AddSection({ Name = "サイズ設定" })
BigHeartTab:AddSlider({ Name = "基本サイズ", Min = 5, Max = 50, Default = BigHeartConfig.Size, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) BigHeartConfig.Size = v end })
BigHeartTab:AddSlider({ Name = "拡大率", Min = 1.0, Max = 10.0, Default = BigHeartConfig.HeartScale, Color = Theme.SliderColor, Increment = 0.1, ValueName = "倍", Callback = function(v) BigHeartConfig.HeartScale = v end })
BigHeartTab:AddSlider({ Name = "縦方向引き伸ばし", Min = 1.0, Max = 5.0, Default = BigHeartConfig.VerticalStretch, Color = Theme.SliderColor, Increment = 0.1, ValueName = "倍", Callback = function(v) BigHeartConfig.VerticalStretch = v end })
BigHeartTab:AddSlider({ Name = "基本高さ", Min = 5, Max = 50, Default = BigHeartConfig.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) BigHeartConfig.Height = v end })
BigHeartTab:AddSlider({ Name = "オブジェクト数", Min = 12, Max = 100, Default = BigHeartConfig.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) BigHeartConfig.ObjectCount = v; if BigHeartConfig.Enabled then toggleBigHeart(false); task.wait(0.1); toggleBigHeart(true) end end })
BigHeartTab:AddSection({ Name = "動き設定" })
BigHeartTab:AddSlider({ Name = "回転速度", Min = 0, Max = BigHeartConfig.RotationSpeedMax, Default = BigHeartConfig.RotationSpeed, Color = Theme.SliderColor, Increment = 0.5, ValueName = "速度", Callback = function(v) BigHeartConfig.RotationSpeed = v end })
BigHeartTab:AddSlider({ Name = "脈動速度", Min = 0, Max = BigHeartConfig.PulseSpeedMax, Default = BigHeartConfig.PulseSpeed, Color = Theme.SliderColor, Increment = 0.5, ValueName = "速度", Callback = function(v) BigHeartConfig.PulseSpeed = v end })
BigHeartTab:AddSlider({ Name = "脈動振幅", Min = 0, Max = 10, Default = BigHeartConfig.PulseAmplitude, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) BigHeartConfig.PulseAmplitude = v end })
...
StarOfDavidTab:AddToggle({ Name = "プレイヤー追従", Default = StarOfDavidConfig.FollowPlayer, Callback = function(v) StarOfDavidConfig.FollowPlayer = v end })
StarOfDavidTab:AddSection({ Name = "サイズ設定" })
StarOfDavidTab:AddSlider({ Name = "星のサイズ", Min = 2, Max = 50, Default = StarOfDavidConfig.Size, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) StarOfDavidConfig.Size = v end })
StarOfDavidTab:AddSlider({ Name = "基本高さ", Min = 0, Max = 50, Default = StarOfDavidConfig.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) StarOfDavidConfig.Height = v end })
StarOfDavidTab:AddSlider({ Name = "三角形の高さ", Min = 0, Max = 20, Default = StarOfDavidConfig.TriangleHeight, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) StarOfDavidConfig.TriangleHeight = v end })
StarOfDavidTab:AddSlider({ Name = "オブジェクト数", Min = 6, Max = 100, Default = StarOfDavidConfig.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) StarOfDavidConfig.ObjectCount = v; if StarOfDavidConfig.Enabled then toggleStarOfDavid(false); task.wait(0.1); toggleStarOfDavid(true) end end })
StarOfDavidTab:AddSection({ Name = "動き設定" })
StarOfDavidTab:AddSlider({ Name = "回転速度", Min = 0, Max = 10, Default = StarOfDavidConfig.RotationSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) StarOfDavidConfig.RotationSpeed = v end })
StarOfDavidTab:AddSlider({ Name = "脈動速度", Min = 0, Max = 10, Default = StarOfDavidConfig.PulseSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) StarOfDavidConfig.PulseSpeed = v end })
...
StarTab:AddToggle({ Name = "プレイヤー追従", Default = StarConfig.FollowPlayer, Callback = function(v) StarConfig.FollowPlayer = v end })
StarTab:AddSection({ Name = "サイズ設定" })
StarTab:AddSlider({ Name = "外側半径", Min = 2, Max = 50, Default = StarConfig.OuterRadius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) StarConfig.OuterRadius = v end })
StarTab:AddSlider({ Name = "内側半径", Min = 1, Max = 30, Default = StarConfig.InnerRadius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) StarConfig.InnerRadius = v end })
StarTab:AddSlider({ Name = "基本高さ", Min = 0, Max = 50, Default = StarConfig.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) StarConfig.Height = v end })
StarTab:AddSlider({ Name = "オブジェクト数", Min = 5, Max = 100, Default = StarConfig.ObjectCount, Color = Theme.SliderColor, Increment = 1, ValueName = "個", Callback = function(v) StarConfig.ObjectCount = v; if StarConfig.Enabled then toggleStar(false); task.wait(0.1); toggleStar(true) end end })
StarTab:AddSection({ Name = "動き設定" })
StarTab:AddSlider({ Name = "回転速度", Min = 0, Max = 10, Default = StarConfig.RotationSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) StarConfig.RotationSpeed = v end })
StarTab:AddSlider({ Name = "きらめき速度", Min = 0, Max = 10, Default = StarConfig.TwinkleSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) StarConfig.TwinkleSpeed = v end })
...
Star2Tab:AddToggle({ Name = "プレイヤー追従", Default = Star2Config.FollowPlayer, Callback = function(v) Star2Config.FollowPlayer = v end })
Star2Tab:AddSection({ Name = "サイズ設定" })
Star2Tab:AddSlider({ Name = "基本サイズ", Min = 5, Max = Star2Config.SizeMax, Default = Star2Config.Size, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) Star2Config.Size = v end })
Star2Tab:AddSlider({ Name = "最大距離制限", Min = 10, Max = 200, Default = Star2Config.MaxDistance, Color = Theme.SliderColor, Increment = 5, ValueName = "スタッド", Callback = function(v) Star2Config.MaxDistance = v end })
Star2Tab:AddSlider({ Name = "光線の長さ", Min = 1.0, Max = Star2Config.RayLengthMax, Default = Star2Config.RayLength, Color = Theme.SliderColor, Increment = 0.5, ValueName = "スタッド", Callback = function(v) Star2Config.RayLength = v end })
Star2Tab:AddSlider({ Name = "基本高さ", Min = 5, Max = 50, Default = Star2Config.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) Star2Config.Height = v end })
Star2Tab:AddSlider({ Name = "光線の数", Min = 6, Max = 36, Default = Star2Config.RayCount, Color = Theme.SliderColor, Increment = 2, ValueName = "本", Callback = function(v) Star2Config.RayCount = v; if Star2Config.Enabled then toggleStar2(false); task.wait(0.1); toggleStar2(true) end end })
Star2Tab:AddSlider({ Name = "オブジェクト数", Min = 12, Max = 100, Default = Star2Config.ObjectCount, Color = Theme.SliderColor, Increment = 4, ValueName = "個", Callback = function(v) Star2Config.ObjectCount = v; if Star2Config.Enabled then toggleStar2(false); task.wait(0.1); toggleStar2(true) end end })
Star2Tab:AddSection({ Name = "動き設定" })
Star2Tab:AddSlider({ Name = "回転速度", Min = 0, Max = Star2Config.RotationSpeedMax, Default = Star2Config.RotationSpeed, Color = Theme.SliderColor, Increment = 1, ValueName = "速度", Callback = function(v) Star2Config.RotationSpeed = v end })
Star2Tab:AddSlider({ Name = "脈動速度", Min = 0, Max = Star2Config.PulseSpeedMax, Default = Star2Config.PulseSpeed, Color = Theme.SliderColor, Increment = 1, ValueName = "速度", Callback = function(v) Star2Config.PulseSpeed = v end })
Star2Tab:AddSlider({ Name = "脈動振幅", Min = 0, Max = 10, Default = Star2Config.PulseAmplitude, Color = Theme.SliderColor, Increment = 0.2, ValueName = "スタッド", Callback = function(v) Star2Config.PulseAmplitude = v end })
Star2Tab:AddSection({ Name = "ギザギザ効果" })
Star2Tab:AddSlider({ Name = "ギザギザ速度", Min = 0, Max = 20, Default = Star2Config.JitterSpeed, Color = Theme.SliderColor, Increment = 0.5, ValueName = "速度", Callback = function(v) Star2Config.JitterSpeed = v end })
Star2Tab:AddSlider({ Name = "ギザギザ量", Min = 0, Max = 10, Default = Star2Config.JitterAmount, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) Star2Config.JitterAmount = v end })
...
SphereTab:AddToggle({ Name = "プレイヤー追従", Default = SphereConfig.FollowPlayer, Callback = function(v) SphereConfig.FollowPlayer = v end })
SphereTab:AddSection({ Name = "サイズ設定" })
SphereTab:AddSlider({ Name = "球体半径", Min = 2, Max = 50, Default = SphereConfig.Radius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) SphereConfig.Radius = v end })
SphereTab:AddSlider({ Name = "基本高さ", Min = -20, Max = 20, Default = SphereConfig.BaseHeight, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) SphereConfig.BaseHeight = v end })
SphereTab:AddSlider({ Name = "緯度線の数", Min = 2, Max = 16, Default = SphereConfig.Latitudes, Color = Theme.SliderColor, Increment = 1, ValueName = "本", Callback = function(v) SphereConfig.Latitudes = v; if SphereConfig.Enabled then toggleSphere(false); task.wait(0.1); toggleSphere(true) end end })
SphereTab:AddSlider({ Name = "経度線の数", Min = 4, Max = 24, Default = SphereConfig.Longitudes, Color = Theme.SliderColor, Increment = 2, ValueName = "本", Callback = function(v) SphereConfig.Longitudes = v; if SphereConfig.Enabled then toggleSphere(false); task.wait(0.1); toggleSphere(true) end end })
SphereTab:AddSlider({ Name = "オブジェクト数", Min = 8, Max = 100, Default = SphereConfig.ObjectCount, Color = Theme.SliderColor, Increment = 4, ValueName = "個", Callback = function(v) SphereConfig.ObjectCount = v; if SphereConfig.Enabled then toggleSphere(false); task.wait(0.1); toggleSphere(true) end end })
SphereTab:AddSection({ Name = "回転設定" })
SphereTab:AddSlider({ Name = "水平回転速度", Min = 0, Max = 10, Default = SphereConfig.HorizontalRotationSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) SphereConfig.HorizontalRotationSpeed = v end })
SphereTab:AddSlider({ Name = "垂直回転速度", Min = 0, Max = 10, Default = SphereConfig.VerticalRotationSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) SphereConfig.VerticalRotationSpeed = v end })
SphereTab:AddSlider({ Name = "螺旋速度", Min = 0, Max = 10, Default = SphereConfig.SpiralSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) SphereConfig.SpiralSpeed = v end })
SphereTab:AddSection({ Name = "効果設定" })
SphereTab:AddSlider({ Name = "波速度", Min = 0, Max = 10, Default = SphereConfig.WaveSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) SphereConfig.WaveSpeed = v end })
SphereTab:AddSlider({ Name = "波振幅", Min = 0, Max = 10, Default = SphereConfig.WaveAmplitude, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) SphereConfig.WaveAmplitude = v end })
SphereTab:AddSlider({ Name = "脈動速度", Min = 0, Max = 10, Default = SphereConfig.PulseSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) SphereConfig.PulseSpeed = v end })
SphereTab:AddSlider({ Name = "脈動振幅", Min = 0, Max = 5, Default = SphereConfig.PulseAmplitude, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) SphereConfig.PulseAmplitude = v end })
...
FerrisWheelTab:AddToggle({ Name = "観覧車起動", Default = false, Callback = toggleFerrisWheel })
FerrisWheelTab:AddToggle({ Name = "プレイヤー追従", Default = FerrisWheelConfig.FollowPlayer, Callback = function(v) FerrisWheelConfig.FollowPlayer = v end })
FerrisWheelTab:AddToggle({ Name = "縦方向円", Default = FerrisWheelConfig.VerticalCircle, Callback = function(v) FerrisWheelConfig.VerticalCircle = v end })

-- 向き設定セクション
FerrisWheelTab:AddSection({ Name = "向き設定" })
...
FerrisWheelTab:AddSlider({ 
    ...
    Color = Theme.SliderColor, Increment = 5, ValueName = "度", 
...
FerrisWheelTab:AddSection({ Name = "サイズ設定" })
FerrisWheelTab:AddSlider({ Name = "半径", Min = 5, Max = 50, Default = FerrisWheelConfig.Radius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) FerrisWheelConfig.Radius = v end })
FerrisWheelTab:AddSlider({ Name = "中心高さ", Min = 5, Max = 50, Default = FerrisWheelConfig.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) FerrisWheelConfig.Height = v end })
FerrisWheelTab:AddSlider({ Name = "オブジェクト数", Min = 6, Max = 100, Default = FerrisWheelConfig.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) FerrisWheelConfig.ObjectCount = v; if FerrisWheelConfig.Enabled then toggleFerrisWheel(false); task.wait(0.1); toggleFerrisWheel(true) end end })
FerrisWheelTab:AddSection({ Name = "動き設定" })
FerrisWheelTab:AddSlider({ Name = "回転速度", Min = 0, Max = FerrisWheelConfig.RotationSpeedMax, Default = FerrisWheelConfig.RotationSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) FerrisWheelConfig.RotationSpeed = v end })
FerrisWheelTab:AddSlider({ Name = "傾き角度", Min = 0, Max = 90, Default = FerrisWheelConfig.TiltAngle, Color = Theme.SliderColor, Increment = 5, ValueName = "度", Callback = function(v) FerrisWheelConfig.TiltAngle = v end })
FerrisWheelTab:AddSection({ Name = "効果設定" })
FerrisWheelTab:AddToggle({ Name = "脈動効果", Default = FerrisWheelConfig.PulseEffect, Callback = function(v) FerrisWheelConfig.PulseEffect = v end })
FerrisWheelTab:AddSlider({ Name = "脈動速度", Min = 0, Max = 10, Default = FerrisWheelConfig.PulseSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) FerrisWheelConfig.PulseSpeed = v end })
FerrisWheelTab:AddSlider({ Name = "脈動振幅", Min = 0, Max = 10, Default = FerrisWheelConfig.PulseAmplitude, Color = Theme.SliderColor, Increment = 0.2, ValueName = "スタッド", Callback = function(v) FerrisWheelConfig.PulseAmplitude = v end })
FerrisWheelTab:AddToggle({ Name = "揺れ効果", Default = FerrisWheelConfig.SwingEffect, Callback = function(v) FerrisWheelConfig.SwingEffect = v end })
FerrisWheelTab:AddSlider({ Name = "揺れの大きさ", Min = 0, Max = 10, Default = FerrisWheelConfig.SwingAmount, Color = Theme.SliderColor, Increment = 0.1, ValueName = "スタッド", Callback = function(v) FerrisWheelConfig.SwingAmount = v end })
...
AnimN1Tab:AddToggle({ Name = "プレイヤー追従", Default = AnimN1Config.FollowPlayer, Callback = function(v) AnimN1Config.FollowPlayer = v end })
AnimN1Tab:AddSection({ Name = "サイズ設定" })
AnimN1Tab:AddSlider({ Name = "半径", Min = 5, Max = 50, Default = AnimN1Config.Radius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN1Config.Radius = v end })
AnimN1Tab:AddSlider({ Name = "基本高さ", Min = 0, Max = 50, Default = AnimN1Config.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN1Config.Height = v end })
AnimN1Tab:AddSlider({ Name = "オブジェクト数", Min = 10, Max = 100, Default = AnimN1Config.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) AnimN1Config.ObjectCount = v; if AnimN1Config.Enabled then toggleAnimN1(false); task.wait(0.1); toggleAnimN1(true) end end })
AnimN1Tab:AddSection({ Name = "アニメーション設定" })
AnimN1Tab:AddSlider({ Name = "回転速度", Min = 0, Max = 50, Default = AnimN1Config.RotationSpeed, Color = Theme.SliderColor, Increment = 1, ValueName = "速度", Callback = function(v) AnimN1Config.RotationSpeed = v end })
AnimN1Tab:AddSlider({ Name = "脈動速度", Min = 0, Max = 20, Default = AnimN1Config.PulseSpeed, Color = Theme.SliderColor, Increment = 0.5, ValueName = "速度", Callback = function(v) AnimN1Config.PulseSpeed = v end })
AnimN1Tab:AddSlider({ Name = "脈動量", Min = 0, Max = 30, Default = AnimN1Config.PulseAmount, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN1Config.PulseAmount = v end })
...
AnimN2Tab:AddToggle({ Name = "プレイヤー追従", Default = AnimN2Config.FollowPlayer, Callback = function(v) AnimN2Config.FollowPlayer = v end })
AnimN2Tab:AddSection({ Name = "サイズ設定" })
AnimN2Tab:AddSlider({ Name = "半径", Min = 3, Max = 30, Default = AnimN2Config.Radius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN2Config.Radius = v end })
AnimN2Tab:AddSlider({ Name = "最低高さ", Min = 0, Max = 20, Default = AnimN2Config.BaseHeight, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN2Config.BaseHeight = v end })
AnimN2Tab:AddSlider({ Name = "最高高さ", Min = 10, Max = 100, Default = AnimN2Config.TopHeight, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN2Config.TopHeight = v end })
AnimN2Tab:AddSlider({ Name = "オブジェクト数", Min = 10, Max = 100, Default = AnimN2Config.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) AnimN2Config.ObjectCount = v; if AnimN2Config.Enabled then toggleAnimN2(false); task.wait(0.1); toggleAnimN2(true) end end })
AnimN2Tab:AddSection({ Name = "アニメーション設定" })
AnimN2Tab:AddSlider({ Name = "回転速度", Min = 0, Max = 50, Default = AnimN2Config.RotationSpeed, Color = Theme.SliderColor, Increment = 1, ValueName = "速度", Callback = function(v) AnimN2Config.RotationSpeed = v end })
AnimN2Tab:AddSlider({ Name = "上昇速度", Min = 0, Max = 10, Default = AnimN2Config.RiseSpeed, Color = Theme.SliderColor, Increment = 0.2, ValueName = "速度", Callback = function(v) AnimN2Config.RiseSpeed = v end })
AnimN2Tab:AddSlider({ Name = "カオス要素", Min = 0, Max = 10, Default = AnimN2Config.ChaosFactor, Color = Theme.SliderColor, Increment = 0.2, ValueName = "強さ", Callback = function(v) AnimN2Config.ChaosFactor = v end })

-- ====================================================================
-- タブ12: アニメーションN3
-- ====================================================================
local AnimN3Tab = Window:MakeTab({
    Name = "N3: ハイパー・エクスプロージョン",
    Icon = "rbxassetid://4483362458",
    PremiumOnly = false
})

AnimN3Tab:AddToggle({ Name = "N3起動", Default = false, Callback = toggleAnimN3 })
AnimN3Tab:AddToggle({ Name = "プレイヤー追従", Default = AnimN3Config.FollowPlayer, Callback = function(v) AnimN3Config.FollowPlayer = v end })
AnimN3Tab:AddSection({ Name = "サイズ設定" })
AnimN3Tab:AddSlider({ Name = "基本高さ", Min = 0, Max = 50, Default = AnimN3Config.Height, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN3Config.Height = v end })
AnimN3Tab:AddSlider({ Name = "爆発半径", Min = 5, Max = 100, Default = AnimN3Config.ExplosionRadius, Color = Theme.SliderColor, Increment = 1, ValueName = "スタッド", Callback = function(v) AnimN3Config.ExplosionRadius = v end })
AnimN3Tab:AddSlider({ Name = "オブジェクト数", Min = 10, Max = 100, Default = AnimN3Config.ObjectCount, Color = Theme.SliderColor, Increment = 2, ValueName = "個", Callback = function(v) AnimN3Config.ObjectCount = v; if AnimN3Config.Enabled then toggleAnimN3(false); task.wait(0.1); toggleAnimN3(true) end end })
AnimN3Tab:AddSection({ Name = "アニメーション設定" })
AnimN3Tab:AddSlider({ Name = "サイクル速度", Min = 0, Max = 5, Default = AnimN3Config.CycleSpeed, Color = Theme.SliderColor, Increment = 0.1, ValueName = "速度", Callback = function(v) AnimN3Config.CycleSpeed = v end })
AnimN3Tab:AddSlider({ Name = "爆発速度", Min = 1, Max = 20, Default = AnimN3Config.ExplosionSpeed, Color = Theme.SliderColor, Increment = 0.5, ValueName = "速度", Callback = function(v) AnimN3Config.ExplosionSpeed = v end })
AnimN3Tab:AddSlider({ Name = "ランダム性", Min = 0, Max = 20, Default = AnimN3Config.Randomness, Color = Theme.SliderColor, Increment = 0.5, ValueName = "強さ", Callback = function(v) AnimN3Config.Randomness = v end })
...
GUITab:AddButton({
    ...
    Callback = function()
        ...
        ScreenGui.ResetOnSpawn = false -- 死んでも消えない
        ...
        MainFrame.Visible = false -- 最初は隠しておく
...
UtilityTab:AddSection({ Name = "移動機能" })
UtilityTab:AddToggle({ Name = "無限ジャンプ", Default = false, Callback = toggleInfiniteJump })
UtilityTab:AddToggle({ Name = "ノークリップ", Default = false, Callback = toggleNoclip })
UtilityTab:AddToggle({ Name = "TPWalk", Default = false, Callback = toggleTPWalk })
UtilityTab:AddSlider({ Name = "TPWalk速度", Min = 16, Max = UtilityConfig.TPWalkSpeedMax, Default = UtilityConfig.TPWalkSpeed, Color = Theme.SliderColor, Increment = 5, ValueName = "速度", Callback = function(v) UtilityConfig.TPWalkSpeed = v; if UtilityConfig.TPWalk then toggleTPWalk(false); task.wait(0.1); toggleTPWalk(true) end end })

UtilityTab:AddSection({ Name = "視覚機能" })
UtilityTab:AddToggle({ Name = "ESP", Default = false, Callback = toggleESP })
UtilityTab:AddSlider({ Name = "FOV", Min = UtilityConfig.MinFOV, Max = UtilityConfig.MaxFOV, Default = UtilityConfig.FOV, Color = Theme.SliderColor, Increment = 1, ValueName = "度", Callback = function(v) UtilityConfig.FOV = v; if Camera then Camera.FieldOfView = v end end })
UtilityTab:AddButton({ Name = "FOVリセット", Callback = function() if Camera then Camera.FieldOfView = UtilityConfig.OriginalFOV; UtilityConfig.FOV = UtilityConfig.OriginalFOV end end })

UtilityTab:AddSection({ Name = "情報" })
UtilityTab:AddLabel("プレイヤー: " .. LocalPlayer.Name)
UtilityTab:AddLabel("対応オブジェクト: " .. #ObjectIDConfig.AvailableObjects .. "種類")
UtilityTab:AddLabel("FOV: " .. UtilityConfig.FOV .. "度")

UtilityTab:AddSection({ Name = "制御" })
UtilityTab:AddButton({ Name = "全機能停止", Callback = function()
    if FeatherConfig.Enabled then toggleFeather(false) end
    if MagicCircleConfig.Enabled then toggleMagicCircle(false) end
    if HeartConfig.Enabled then toggleHeart(false) end
    if BigHeartConfig.Enabled then toggleBigHeart(false) end
...
ScriptHubTab:AddButton({
    Name = "シェーダー",
    Callback = function()
        executeScript("https://rawscripts.net/raw/Universal-Script-Shader-77482", "シェーダー")
    end
})
...
ScriptHubTab:AddButton({
    Name = "空変え",
    Callback = function()
        executeScript("https://rawscripts.net/raw/Universal-Script-SkyBoxinjectHUB-80671", "空変え")
    end
})
...
ScriptHubTab:AddButton({
    Name = "テトリス",
    Callback = function()
        executeScript("https://rawscripts.net/raw/Universal-Script-RTetris-76191", "テトリス")
    end
})
...
ScriptHubTab:AddButton({
    Name = "クロスケ作v式飛行",
    Callback = function()
        executeScript("https://rawscripts.net/raw/Universal-Script-VFly-gui-and-noclip-78112", "クロスケ作v式飛行")
    end
})
...
ScriptHubTab:AddButton({
    Name = "Hokuto hub",
    Callback = function()
        -- Hokuto.hub ESP Script with Fixed Aim Systems (無限範囲バージョン)
        -- Delta/Xeno用
        ...
        -- ESP設定（指定カラーのみ）
        local settings = {
            toggleKey = Enum.KeyCode.RightShift, -- すべてのGUI表示切り替えキー
            guiColor = Color3.fromRGB(255, 105, 180), -- ピンク
            textColor = Color3.fromRGB(255, 255, 255), -- 白
            backgroundColor = Color3.fromRGB(20, 20, 20), -- 黒
            lineColor = Color3.fromRGB(255, 165, 0), -- オレンジ（線のみ）
            transparency = 0.1 -- 低い透明度
        }
        ...
        -- ===== 修正箇所: Aimbotロジック（無限範囲、近いプレイヤー優先）=====
        ...
        local function initialize()
            ...
            if espHandles.aimbotButton then
                espHandles.aimbotButton.MouseButton1Click:Connect(function()
                    ...
                    addSeisaLogMessage(seisaLogHandles, "Aimbot " .. (AimbotEnabled and "enabled (無限範囲)" or "disabled"))
            ...
            addSeisaLogMessage(seisaLogHandles, "Aim systems ready - 無限範囲")
            ...
            print("  ✓ AIMBOT - 無限範囲・最も近いプレイヤーを自動追尾")
            print("  ✓ AIMROOK - 手動で選択したプレイヤーを追尾")
...
InfoTab:AddLabel("バージョン: V0.6")
InfoTab:AddLabel("開発グループ: 土星グループ")

InfoTab:AddSection({ Name = "リンク" })
InfoTab:AddButton({
    Name = "サポートサーバーに参加[ディスコード]",
    Callback = function()
        copyToClipboard("https://discord.gg/6u4pjQrEsC", "ディスコードリンク")
    end
})

InfoTab:AddButton({
    Name = "サポートサーバーに参加[ラインオープンチャット]",
    Callback = function()
        copyToClipboard("https://line.me/ti/g2/-CBPty1N0v2XuNPqaJm5xLmCBt0wBr3_vxrOqQ?utm_source=invitation&utm_medium=link_copy&utm_campaign=default", "ラインリンク")
    end
})
...
task.wait(1)
OrionLib:MakeNotification({
    Name = "🌸さくらhub🌸 v0.6.jp 起動完了",
    Content = "全18タブ、ピンクグラデーション適用、バリア破壊追加",
    Image = "rbxassetid://4483362458",
    Time = 5
})

print("==========================================")
print("🌸さくらhub🌸 v0.6.jp")
print("==========================================")
print("対応オブジェクト総数: " .. #ObjectIDConfig.AvailableObjects .. "種類")
print("全タブ数: 18")
print("最大オブジェクト数: 100")
print("観覧車: ねじれ修正済、固定方向対応")
print("新機能: ピンクグラデーション自動適用")
print("新規タブ: GUI, スクリプトhub, バリア破壊, 情報とサポート")
print("==========================================")
