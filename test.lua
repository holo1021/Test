                            ...
                            if plot then
                                ...
                                if owners then
                                    ...
                                    if ownerName == LocalPlayer.Name then
                                        ...
                                        if timeVal <= 5 and timeVal > 0 then
...
local function StartHolonHUB()
    ...
    -- [Grab Mod Logic Integration]
    local function getGrabNotify()
        local ps = LocalPlayer:WaitForChild("PlayerScripts", 10)
        if not ps then return nil end
        local cbm = ps:WaitForChild("CharacterAndBeamMove", 10)
        if not cbm then return nil end
        return cbm:WaitForChild("GrabNotifyEvent", 10)
    end

    local grabNotify = getGrabNotify()
    local lastGrabbedPart = nil
    local grabOriginalCollisions = {}

    local function findGrabbedTarget()
    ...
    if grabNotify then
        grabNotify.Event:Connect(function(isGrabbing)
            if isGrabbing then
                local target = findGrabbedTarget()
                if not target then
                    for i = 1, 20 do
                        RunService.Heartbeat:Wait()
                        target = findGrabbedTarget()
                        if target then break end
                    end
                end

                if target then
                    lastGrabbedPart = target
                    
                    if cfg.GrabMod.Kill then
                        local char = target.Parent
                        local hum = char:FindFirstChild("Humanoid") or char.Parent:FindFirstChild("Humanoid")
                        if hum then hum.Health = 0 end
                    end

                    if cfg.GrabMod.Noclip then
                        local root = target.Parent:IsA("Model") and target.Parent or target
                        pcall(function()
                            for _, p in pairs(root:GetDescendants()) do
                                if p:IsA("BasePart") then
                                    grabOriginalCollisions[p] = p.CanCollide
                                    p.CanCollide = false
                                end
                            end
                        end)
                    ...
                    if cfg.GrabMod.Spin then
                        task.spawn(function()
                            local partToSpin = lastGrabbedPart
                            while partToSpin and partToSpin.Parent and cfg.GrabMod.Spin do
                                pcall(function()
                                    partToSpin.AssemblyAngularVelocity = Vector3.new(0, 100, 0)
                                end)
                                RunService.Heartbeat:Wait()
                            end
                            if partToSpin and partToSpin.Parent then
                                pcall(function() partToSpin.AssemblyAngularVelocity = Vector3.zero end)
                            end
                        end)
                    end
                end
            else
                if lastGrabbedPart then
                    -- 左クリックしている時だけ投げる (ドロップ時は反応しない)
                    if cfg.GrabMod.SuperThrow and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                        pcall(function()
                            local force = Camera.CFrame.LookVector * cfg.GrabMod.ThrowPower
                            lastGrabbedPart:ApplyImpulse(force * lastGrabbedPart:GetMass())
                        end)
                    end
                    task.delay(0.1, function() lastGrabbedPart = nil end)
                end
                for part, state in pairs(grabOriginalCollisions) do
                    if part and part.Parent then
                        pcall(function() part.CanCollide = state end)
                    end
                end
                grabOriginalCollisions = {}
            end
        end)
    end
...
GrabTab:AddToggle({
    Name = "掴んだらキル",
    Default = cfg.GrabMod.Kill,
    Callback = function(v) cfg.GrabMod.Kill = v end
})

GrabTab:AddToggle({
    Name = "掴んでいる間すり抜け (Noclip)",
    Default = cfg.GrabMod.Noclip,
    Callback = function(v) cfg.GrabMod.Noclip = v end
})

GrabTab:AddSection({ Name = "Spin Settings" })

GrabTab:AddToggle({
    Name = "掴んだら回転",
    Default = cfg.GrabMod.Spin,
    Callback = function(v) cfg.GrabMod.Spin = v end
})
...
GrabTab:AddToggle({
    Name = "スーパースロー (投げ飛ばし)",
    Default = cfg.GrabMod.SuperThrow,
    Callback = function(v) cfg.GrabMod.SuperThrow = v end
})
...
GrabTab:AddSlider({
    ...
    Min = 0,
    Max = 5000,
    Default = cfg.GrabMod.ThrowPower,
    Color = Color3.fromRGB(255, 255, 255),
    Increment = 100,
    ValueName = "Power",
    Callback = function(v) cfg.GrabMod.ThrowPower = v end
})
...
UserInputService.InputChanged:Connect(function(inputObject, gameProcessed)
    if _G.FutherExtend and inputObject.UserInputType == Enum.UserInputType.MouseWheel then
...
Workspace.ChildAdded:Connect(function(potentialGrabPart)
    if potentialGrabPart.Name == "GrabParts" and potentialGrabPart:IsA("Model") then
        if _G.FutherExtend then
            local grabPartModel = potentialGrabPart
            local grabPart = grabPartModel:WaitForChild("GrabPart", 5)
            local dragPart = grabPartModel:WaitForChild("DragPart", 5)
